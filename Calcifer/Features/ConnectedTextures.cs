...
class ConnectedTextures
{
    ...
    private static void UpdateNeighbours(GameLocation where, Rectangle bounds)
    {
        foreach (Vector2 neighbourTile in Neighbour_All(bounds))
        {
            ...
            if (where.furniture.FirstOrDefault(f => Furniture_ContainsAndAligned(f, neighbourTile, bounds)) is Furniture neighbourFurniture)
            {
    ...
    private static bool ConnectsToTile(GameLocation where, Vector2 tile, Rectangle bounds, IList<string> connections)
    {
        ...
        // furniture
        if (where.furniture.FirstOrDefault(f => Furniture_ContainsAndAligned(f, tile, bounds)) is Furniture furniture && Connects(furniture.QualifiedItemId, connections))
            return true;
